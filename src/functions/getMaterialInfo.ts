...
export async function get_material_info() {
	...
	useMaterialEditStore.setState((state) => {
		...
		for (let store_key in state.material_info) {
			for (let res_key in response) {
				if (store_key === res_key) {
					(state.material_info as any)[store_key] = (response as any)[
						res_key
					];
				}
			}
		...
		state.first_image_set.first_image_title =
			state.material_info.material_source_file_count.toString() + '套 ';
