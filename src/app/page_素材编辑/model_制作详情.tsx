			...
			<Grid2 container spacing={2}>
							...
							<Select
								...
								onChange={(event) =>
									useMaterialEditStore.setState((state) => {
										...
										(state.product_image_set as any)[
											item.key
										] = event.target.value;
									})
								}
				...
				<Grid2 xs={2}>
					<TextField
						...
						onChange={(event) => {
							useMaterialEditStore.setState((state) => {
								state.product_image_set.preview_used_number =
					...
					await DefaultService.makeMaterialProductImageV1MakeMaterialProductImagePost(
						{
							...
							effect_image_path:
								store.material_info.effect_image_path,
							has_effect_image:
								store.product_image_set.has_effect_image,
							has_preview_image:
								store.product_image_set.has_preview_image,
							...
							material_source_format_title:
								store.material_info
									.material_source_format_title,
							oneline_number:
								store.product_image_set.oneline_number,
							oneline_ratio:
								store.product_image_set.oneline_ratio,
							preview_has_material_info:
								store.product_image_set
									.preview_has_material_info,
							preview_image_count:
								store.material_info.preview_image_count,
							preview_image_path:
								store.material_info.preview_image_path,
							preview_image_sort:
								store.product_image_set.preview_image_sort,
							preview_used_number:
